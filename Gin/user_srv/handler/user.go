...
type UserRegisterServer struct {
	userproto.UnimplementedUserRegisterServer
}

func (s *UserRegisterServer) Register(ctx context.Context, req *userproto.DouyinUserRegisterRequest) (*userproto.DouyinUserRegisterResponse, error) {
	username, password := req.Username, req.Password
	var user model.User
	result := global.DB.Where(&model.User{UserName: username}).First(&user)
	if result.RowsAffected == 1 {
		return &userproto.DouyinUserRegisterResponse{
			StatusCode: 1,
			StatusMsg:  "用户名已存在",
		}, nil
	}
...
func (s *UserRegisterServer) Login(ctx context.Context, req *userproto.DouyinUserRegisterRequest) (*userproto.DouyinUserRegisterResponse, error) {
	username, password := req.Username, req.Password
	var user model.User
	result := global.DB.Where(&model.User{UserName: username}).First(&user)
...
func (s *UserRegisterServer) GetUserById(ctx context.Context, req *userproto.IdRequest) (*userproto.User, error) {
	if req.NeedToken {
